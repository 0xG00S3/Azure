...
function Get-KeyVaultSignedJwt {
    ...
    $httpResponse = Invoke-RestMethod -Uri $uri -Headers @{ 'Authorization' = "Bearer $($KeyToken -replace '\r|\n','')" }
    $keycert = $httpResponse

    $certBytes = [System.Convert]::FromBase64String($keycert.cer)
    $certObj = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2 -ArgumentList @(,$certBytes)
    $certificateHash = [System.Convert]::ToBase64String($certObj.GetCertHash()) -replace '\+','-' -replace '/','_' -replace '='
...
$signUri = "https://$KeyVaultName.vault.azure.net/keys/$CertName/sign?api-version=7.3"
$signResponse = Invoke-RestMethod -Uri $signUri -Method POST -Headers @{
    'Authorization' = "Bearer $($KeyToken -replace '\r|\n','')"
...
$signature = $signResponse.value -replace '\+','-' -replace '/','_' -replace '='  
